...
async def send_eternally(sock):
    ...
    """
    while True:
        # v 0.12.0 gives Depricated on this
        stuff = await trio.run_sync_in_worker_thread(input, cancellable=True)
        #stuff = await trio.to_thread.to_thread_run_sync(input, cancellable=True)
