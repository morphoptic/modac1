...
class DataModel() :
    ...
    def __init__(self, window):
        ...
        self.listStore = Gtk.ListStore(str,str,str,str) #str,float, float, float) #gobject.TYPE_STRING,gobject.TYPE_FLOAT,gobject.TYPE_FLOAT,gobject.TYPE_FLOAT)
    ...
    def getBMEData(self):
        ...
        pStr = '%0.3f hPa'% self.bme.pressure
        #
...
class DataWindow(Gtk.Window):
    def __init__(self):
       ...
       #Then we have to create buttons and add them to the toolbar on position 0 and 1. Then add the toolbar to our layout. The two buttons are also connected to their respective function when they are clicked.
        ...
        viewModel = self.dataModel.listStore

        self.treeview = Gtk.TreeView.new_with_model(self.dataModel.listStore)
        for i in range( len(columnNames)) :
            column = Gtk.TreeViewColumn(columnNames[i], Gtk.CellRendererText())
            column.set_min_width(100)
            column.set_alignment(0.5)
            self.treeview.append_column(column)
        self.dataModel.getBMEData() # put one item in data
        ...
        self.line, = self.ax.plot(self.dataModel.times, self.dataModel.temps)  # plot the first row
    ...
    def plotTemp(self): #, treeview, path, view_column):
        ...
        _max = np.amax(self.dataModel.temps)
        if _max < 35:
            _max = 35
    ...
    def plotHumid(self): #, treeview, path, view_column):
        self.line.set_ydata(self.dataModel.humid)
        self.canvas.draw()
        
    def plotPressure(self): #, treeview, path, view_column):
        self.line.set_ydata(self.dataModel.press)
        self.canvas.draw()
        
    def getNextData(self):
        self.dataModel.getBMEData()
        self.plotTemp()
        return True

    def printList(self,widget):
        for row in self.dataModel.listStore :
            print(row[:])
